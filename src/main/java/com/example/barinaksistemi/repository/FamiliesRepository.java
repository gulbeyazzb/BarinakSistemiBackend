...
public interface FamiliesRepository extends JpaRepository<Families,Long> {
    @Query(value = "select * from barinak.families as f where f.id=(select af.family_id " +
            "from barinak.animal_family as af where af.animal_id=:animalId)",nativeQuery = true)
